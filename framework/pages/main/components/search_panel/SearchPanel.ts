...
export class SearchPanel extends BasePage implements ILoadable {
  ...
  @Step('Open {0}')
  async openSelector(
    type: (typeof SearchPanelSelectors)[keyof typeof SearchPanelSelectors],
  ): Promise<
    | DepartureSelectorComponent
    | DestinationListSelectorComponent
    | DepartureDateSelectorComponent
    | RoomsAndGuestsSelectorComponent
  > {
    switch (type) {
  ...
  async performSearch(criteria: SearchCriteria) {
    if (criteria.departureAirport) {
      ...
      const airport =
        criteria.departureAirport === 'first'
          ? airports.at(0)!
