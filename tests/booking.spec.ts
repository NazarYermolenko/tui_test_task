...
tuiFixture('Perform all passenger validations', async ({ mainPage }) => {
  ...
  await bookPage.clickContinue({ expectSuccess: false });
  await expect
    .soft(leadForm.locators.firstNameError())
    .toContainText(PassengerValidationMessages.NAME_LENGTH_AND_CHARS);
